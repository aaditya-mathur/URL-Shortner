...
export const createUrl = asyncHandler(async (req: any, res: Response) => {
  ...
  const newShortCode = await createShortCode(
    targetUrl,
    userId,
    shortCode as string
  );
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { newShortCode }, "new shortcode created"));
});

// TO GET ALL THE URLS BY THE LOGGED IN USER
export const getallUrlsByTheUser = asyncHandler(
...
// TO DELETE THE EXISTING URL
export const deleteEntryByTheUser = asyncHandler(
  async (req: any, res: Response) => {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { deletedUrl }, "url successfully deleted"));
  }
);
...
export const updateUrlEntry = asyncHandler(async (req: any, res: Response) => {
  ...
  const validationResult = await updatePostRequestBodySchema.safeParseAsync(
    req.body
  );
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { updatedUrl }, "url successfully updated"));
});

// TO GET A PARTICULAR URL BY ID
